//
//  KMutiTitleDisplayViewController.h
//  CollectionViewDemo
//
...
// 颜色渐变样式枚举
typedef NS_ENUM(NSUInteger, KTitleColorGradientStyle) {
    KTitleColorGradientStyleRGB,
    KTitleColorGradientStyleFill,
};
/*
 使用注意：
 1.字体放大效果、遮罩效果 ---- >不能和角标同时使用。
 2.网易效果：颜色渐变 + 字体缩放
 3.进入头条效果：颜色填充渐变
...
@interface KMutiTitleDisplayViewController : UIViewController
...
// *  @author Kael
// *
// *  @brief 是否设置标题为固定宽度（不是自适应） 考虑到适配问题这个暂不开放
// */
//@property (nonatomic,assign) BOOL isStaticW;
...
/**
 颜色渐变样式
 */
@property (nonatomic, assign) KTitleColorGradientStyle titleColorGradientStyle;
...
// 一次性设置所有颜色渐变属性
- (void)setUpTitleGradient:(void(^)(BOOL *isShowTitleGradient,KTitleColorGradientStyle *titleColorGradientStyle,CGFloat *startR,CGFloat *startG,CGFloat *startB,CGFloat *endR,CGFloat *endG,CGFloat *endB))titleGradientBlock;
