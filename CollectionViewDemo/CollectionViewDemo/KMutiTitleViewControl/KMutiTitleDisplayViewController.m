//
//  KMutiTitleDisplayViewController.m
//  CollectionViewDemo
//
...
#import "KDisplayHeader.h"
#import "UIView+Frame.h"
#import "KFlowLayout.h"


@interface KMutiTitleDisplayViewController()<UICollectionViewDelegate,UICollectionViewDataSource>{
...
@implementation KMutiTitleDisplayViewController
#pragma mark - 初始化方法
...
{
    // 初始化标题高度
    _titleHeight = KTitleScrollViewHeigh;
...
- (void)setUp
{
    if (_isShowTitleGradient && _titleColorGradientStyle == KTitleColorGradientStyleRGB) {
...
- (UIScrollView *)contentScrollView
{
    if (_contentScrollView == nil) {
        ...
        // 创建布局
        KFlowLayout *layout = [[KFlowLayout alloc] init];
...
// 一次性设置所有颜色渐变属性
- (void)setUpTitleGradient:(void (^)(BOOL *, KTitleColorGradientStyle *, CGFloat *, CGFloat *, CGFloat *, CGFloat *, CGFloat *, CGFloat *))titleGradientBlock
{
...
- (void)viewDidLayoutSubviews
{
    ...
    CGFloat contentY = self.navigationController?KNavBarHeigh : [UIApplication sharedApplication].statusBarFrame.size.height;
    CGFloat contentW = KScreenW;
    CGFloat contentH = KScreenH - contentY;
    // 设置整个内容的尺寸
    ...
    // 设置标题滚动视图frame
    // 计算尺寸
    CGFloat titleH = _titleHeight?_titleHeight:KTitleScrollViewHeigh;
    ...
    self.contentScrollView.frame = _isfullScreen?CGRectMake(0, 0, contentW, KScreenH) :CGRectMake(0, contentScrollY, contentW, self.contentView.height - contentScrollY);
...
- (void)setUpTitleWidth
{
    ...
    for (NSString *title in titles) {
        ...
        CGFloat width = isStaticTitleWidth ? staticTitleWidth : titleBounds.size.width;
...
- (void)setUpAllTitle
{
    ...
    for (int i = 0; i < count; i++) {
        ...
        UILabel *label = [[KDisplayTitleLabel alloc] init];
    ...
    _contentScrollView.contentSize = CGSizeMake(count * KScreenW, 0);
    
}

#pragma mark - 标题效果渐变方法
// 设置标题颜色渐变
- (void)setUpTitleColorGradientWithOffset:(CGFloat)offsetX rightLabel:(KDisplayTitleLabel *)rightLabel leftLabel:(KDisplayTitleLabel *)leftLabel
{
...
- (void)titleClick:(UITapGestureRecognizer *)tap
{
    ...
    if (vc.view) {
        // 发出通知点击标题通知
        [[NSNotificationCenter defaultCenter] postNotificationName:KDisplayViewClickOrScrollDidFinshNote  object:vc];
        
        // 发出重复点击标题通知
        if (_selIndex == i) {
            [[NSNotificationCenter defaultCenter] postNotificationName:KDisplayViewRepeatClickTitleNote object:vc];
        }
    }
...
- (void)selectLabel:(UILabel *)label
{
    ...
    for (KDisplayTitleLabel *labelView in self.titleLabels) {
    ...
    // 标题缩放
    if (_isShowTitleScale && _titleColorGradientStyle == KTitleColorGradientStyleRGB) {
...
- (void)setUpCoverView:(UILabel *)label
{
    ...
    CGFloat coverW = isStaticTitleWidth ? staticTitleWidth : titleBounds.size.width + 2 * border;
...
- (void)setUpUnderLine:(UILabel *)label
{
    ...
    CGFloat underLineH = _underLineH?_underLineH:KUnderLineHeigh;
    ...
    if (self.underLine.x == 0) {
        self.underLine.width = isStaticTitleWidth ? staticTitleWidth : titleBounds.size.width;
...
- (void)setLabelTitleCenter:(UILabel *)label
{
    ...
    // 计算下最大的标题视图滚动区域
    CGFloat maxOffsetX = self.titleScrollView.contentSize.width - KScreenW + _titleMargin;
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    // 左边按钮
    KDisplayTitleLabel *leftLabel = self.titleLabels[leftIndex];
    
    // 右边角标
    NSInteger rightIndex = leftIndex + 1;
    
    // 右边按钮
    KDisplayTitleLabel *rightLabel = nil;
